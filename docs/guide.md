...
What the platform does, how each piece behaves, and what each one was proved against. Written as it
was built, so it keeps the reasoning rather than only the result.

For the exhaustive list of commands and flags, see the [CLI reference](cli.md). For the shape of the
code and the boundaries the tests enforce, see [ARCHITECTURE.md](ARCHITECTURE.md).
